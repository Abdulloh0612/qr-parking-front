import { useState } from 'react'
import { useSearchParams } from 'react-router-dom'
import { motion, AnimatePresence } from 'framer-motion'
import { Header } from '@/components/layout/Header'
import { Container } from '@/components/layout/Container'
import { Footer } from '@/components/layout/Footer'
import { StepIndicator } from '@/components/forms/StepIndicator'
import { Input } from '@/components/ui/Input'
import { Button } from '@/components/ui/Button'
import { Card } from '@/components/ui/Card'
import { PageTransition } from '@/components/animations/PageTransition'
import { useRegistration } from '@/features/registration/hooks/useRegistration'
import { validateStep1, validateStep2 } from '@/features/registration/validation'
import type { RegistrationData } from '@/features/registration/types'
import { APP_CONFIG } from '@/lib/constants'

function SocialIcon({ type }: { type: string }) {
  if (type === 'whatsapp') return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
    </svg>
  )
  if (type === 'instagram') return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
    </svg>
  )
  if (type === 'telegram') return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
    </svg>
  )
  if (type === 'vk') return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M15.684 0H8.316C1.592 0 0 1.592 0 8.316v7.368C0 22.408 1.592 24 8.316 24h7.368C22.408 24 24 22.408 24 15.684V8.316C24 1.592 22.391 0 15.684 0zm3.692 17.123h-1.744c-.66 0-.864-.525-2.05-1.727-1.033-1-1.49-1.135-1.744-1.135-.356 0-.458.102-.458.593v1.575c0 .424-.135.678-1.253.678-1.846 0-3.896-1.118-5.335-3.202C4.624 10.857 4.03 8.57 4.03 8.096c0-.254.102-.491.593-.491h1.744c.44 0 .61.203.78.677.864 2.49 2.303 4.675 2.896 4.675.22 0 .322-.102.322-.66V9.721c-.068-1.186-.695-1.287-.695-1.71 0-.203.17-.407.44-.407h2.744c.373 0 .508.203.508.643v3.473c0 .372.17.508.271.508.22 0 .407-.136.813-.542 1.254-1.406 2.151-3.574 2.151-3.574.119-.254.322-.491.762-.491h1.744c.525 0 .643.27.525.643-.22 1.017-2.354 4.031-2.354 4.031-.186.305-.254.44 0 .78.186.254.796.779 1.203 1.253.745.847 1.32 1.558 1.473 2.05.17.49-.085.745-.576.745z"/>
    </svg>
  )
  if (type === 'facebook') return (
    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
      <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
    </svg>
  )
  return null
}

const socialColors: Record<string, string> = {
  whatsapp: 'text-green-600',
  instagram: 'text-pink-600',
  telegram: 'text-sky-500',
  vk: 'text-blue-700',
  facebook: 'text-blue-800',
}

function SocialInput({
  type,
  label,
  placeholder,
  value,
  onChange,
  error,
  inputType,
}: {
  type: string
  label: string
  placeholder: string
  value: string
  onChange: (v: string) => void
  error?: string
  inputType?: string
}) {
  return (
    <div className="space-y-1">
      <label className="text-sm font-medium text-gray-700">{label}</label>
      <div className="relative">
        <div className={`absolute left-3 top-1/2 -translate-y-1/2 ${socialColors[type] || 'text-gray-500'}`}>
          <SocialIcon type={type} />
        </div>
        <input
          type={inputType || 'text'}
          placeholder={placeholder}
          value={value}
          onChange={(e) => onChange(e.target.value)}
          className={`w-full pl-10 pr-4 py-3 rounded-xl border bg-white/80 backdrop-blur-sm
            focus:outline-none focus:ring-2 focus:ring-primary-400 focus:border-transparent
            transition-all duration-200 placeholder:text-gray-400
            ${error ? 'border-red-400' : 'border-gray-200'}`}
        />
      </div>
      {error && <p className="text-xs text-red-500">{error}</p>}
    </div>
  )
}

export function RegisterPage() {
  const [searchParams] = useSearchParams()
  const qrId = searchParams.get('qr')

  if (!qrId) {
    return (
      <PageTransition>
        <div className="min-h-screen flex flex-col">
          <Header title="Регистрация QR" />
          <Container className="flex-1 flex items-center justify-center">
            <Card className="text-center">
              <div className="text-red-500 mb-4">
                <svg className="w-16 h-16 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                </svg>
              </div>
              <h2 className="text-xl font-bold mb-2">QR код не найден</h2>
              <p className="text-gray-600">
                Для регистрации отсканируйте QR код на наклейке
              </p>
            </Card>
          </Container>
          <Footer />
        </div>
      </PageTransition>
    )
  }

  const [step, setStep] = useState(1)
  const [formData, setFormData] = useState<RegistrationData>({
    phone: '',
    first_name: '',
    last_name: '',
    vehicle_number: '',
    vehicle_brand: '',
    whatsapp: '',
    instagram: '',
    telegram: '',
    vk: '',
    facebook: '',
    photo_url: '',
    telegram_bot_enabled: false,
  })
  const [errors, setErrors] = useState<Record<string, string>>({})

  const { submitRegistration, loading, error } = useRegistration(qrId || 'DEFAULT')

  const updateField = (field: keyof RegistrationData, value: string | boolean) => {
    setFormData((prev) => ({ ...prev, [field]: value }))
    if (errors[field as string]) {
      setErrors((prev) => ({ ...prev, [field as string]: '' }))
    }
  }

  const handleNextStep = () => {
    if (step === 1) {
      const validationErrors = validateStep1(formData)
      if (Object.keys(validationErrors).length > 0) {
        setErrors(validationErrors)
        return
      }
      setStep(2)
    } else if (step === 2) {
      const validationErrors = validateStep2(formData)
      if (Object.keys(validationErrors).length > 0) {
        setErrors(validationErrors)
        return
      }
      setStep(3)
    }
  }

  const handleSubmit = () => {
    submitRegistration(formData)
  }

  return (
    <PageTransition>
      <div className="min-h-screen flex flex-col">
        <Header title="Регистрация QR" />
        <Container className="py-8 flex-1">
          <StepIndicator
            currentStep={step}
            totalSteps={3}
            labels={['Основное', 'Контакты', 'Telegram']}
          />

          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div
                key="step1"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
              >
                <Card>
                  <h2 className="text-2xl font-bold mb-6">Основная информация</h2>
                  <div className="space-y-4">
                    <Input
                      label="Номер телефона"
                      type="tel"
                      placeholder="+996 XXX XXX XXX"
                      value={formData.phone}
                      onChange={(e) => updateField('phone', e.target.value)}
                      error={errors.phone}
                    />
                    <div className="grid grid-cols-2 gap-4">
                      <Input
                        label="Имя"
                        placeholder="Азамат"
                        value={formData.first_name}
                        onChange={(e) => updateField('first_name', e.target.value)}
                        error={errors.first_name}
                      />
                      <Input
                        label="Фамилия"
                        placeholder="Исаков"
                        value={formData.last_name}
                        onChange={(e) => updateField('last_name', e.target.value)}
                        error={errors.last_name}
                      />
                    </div>
                    <Input
                      label="Номер машины"
                      placeholder="01KG123ABC"
                      value={formData.vehicle_number}
                      onChange={(e) => updateField('vehicle_number', e.target.value.toUpperCase())}
                      error={errors.vehicle_number}
                    />
                    <Input
                      label="Марка машины"
                      placeholder="Toyota Camry"
                      value={formData.vehicle_brand}
                      onChange={(e) => updateField('vehicle_brand', e.target.value)}
                      error={errors.vehicle_brand}
                    />
                  </div>
                  <Button onClick={handleNextStep} className="w-full mt-6">
                    Далее
                  </Button>
                </Card>
              </motion.div>
            )}

            {step === 2 && (
              <motion.div
                key="step2"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
              >
                <Card>
                  <h2 className="text-2xl font-bold mb-1">Социальные сети</h2>
                  <p className="text-gray-500 text-sm mb-6">Необязательно — чтобы другие могли вас найти</p>
                  <div className="space-y-4">
                    <SocialInput
                      type="whatsapp"
                      label="WhatsApp"
                      placeholder="+996 XXX XXX XXX"
                      value={formData.whatsapp}
                      onChange={(v) => updateField('whatsapp', v)}
                      error={errors.whatsapp}
                      inputType="tel"
                    />
                    <SocialInput
                      type="instagram"
                      label="Instagram"
                      placeholder="username"
                      value={formData.instagram}
                      onChange={(v) => updateField('instagram', v)}
                      error={errors.instagram}
                    />
                    <SocialInput
                      type="telegram"
                      label="Telegram"
                      placeholder="@username или +996 XXX XXX XXX"
                      value={formData.telegram}
                      onChange={(v) => updateField('telegram', v)}
                      error={errors.telegram}
                    />
                    <SocialInput
                      type="vk"
                      label="VK"
                      placeholder="username"
                      value={formData.vk}
                      onChange={(v) => updateField('vk', v)}
                      error={errors.vk}
                    />
                    <SocialInput
                      type="facebook"
                      label="Facebook"
                      placeholder="username"
                      value={formData.facebook}
                      onChange={(v) => updateField('facebook', v)}
                      error={errors.facebook}
                    />
                  </div>
                  <div className="flex gap-3 mt-6">
                    <Button variant="secondary" onClick={() => setStep(1)} className="flex-1">
                      Назад
                    </Button>
                    <Button onClick={handleNextStep} className="flex-1">
                      Далее
                    </Button>
                  </div>
                </Card>
              </motion.div>
            )}

            {step === 3 && (
              <motion.div
                key="step3"
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                exit={{ opacity: 0, x: -20 }}
              >
                <Card>
                  <h2 className="text-2xl font-bold mb-2">Telegram бот</h2>
                  <p className="text-gray-500 text-sm mb-6">
                    Получайте уведомления в Telegram, когда кто-то сканирует ваш QR
                  </p>
                  <div className="space-y-4">
                    <label className="flex items-start gap-3 p-4 glass rounded-xl cursor-pointer hover:bg-white/60 transition-colors">
                      <input
                        type="checkbox"
                        checked={formData.telegram_bot_enabled}
                        onChange={(e) => updateField('telegram_bot_enabled', e.target.checked)}
                        className="w-5 h-5 mt-0.5 text-primary-600 rounded focus:ring-2 focus:ring-primary-500 flex-shrink-0"
                      />
                      <div>
                        <p className="font-medium">Активировать Telegram бота</p>
                        <p className="text-sm text-gray-600 mt-0.5">
                          Перейдите в бота и нажмите Start, чтобы получать уведомления о сканированиях и сообщениях
                        </p>
                      </div>
                    </label>

                    {formData.telegram_bot_enabled && (
                      <motion.div
                        initial={{ opacity: 0, y: -10 }}
                        animate={{ opacity: 1, y: 0 }}
                      >
                        <a
                          href={`https://t.me/${APP_CONFIG.telegramBotUsername}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-center gap-4 p-4 bg-sky-50 border border-sky-200 rounded-xl hover:bg-sky-100 transition-colors"
                        >
                          <div className="w-12 h-12 rounded-full bg-sky-500 flex items-center justify-center text-white flex-shrink-0">
                            <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
                              <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z" />
                            </svg>
                          </div>
                          <div>
                            <p className="font-semibold text-sky-700">Открыть @{APP_CONFIG.telegramBotUsername}</p>
                            <p className="text-sm text-sky-600">Нажмите кнопку START в боте</p>
                          </div>
                          <svg className="w-5 h-5 text-sky-400 ml-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                          </svg>
                        </a>
                      </motion.div>
                    )}
                  </div>

                  {error && (
                    <div className="mt-4 p-4 bg-red-50 border border-red-200 rounded-xl">
                      <p className="text-sm text-red-600">{error}</p>
                    </div>
                  )}

                  <div className="flex gap-3 mt-6">
                    <Button variant="secondary" onClick={() => setStep(2)} className="flex-1">
                      Назад
                    </Button>
                    <Button onClick={handleSubmit} loading={loading} className="flex-1">
                      Завершить
                    </Button>
                  </div>
                </Card>
              </motion.div>
            )}
          </AnimatePresence>
        </Container>
        <Footer />
      </div>
    </PageTransition>
  )
}
